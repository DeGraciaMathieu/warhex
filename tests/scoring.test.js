...
describe("système de points", () => {
    ...
    it("la partie ne se termine pas avant le tour 6", () => {
        expect(checkWinner({ 1: 5, 2: 0 }, 5)).toBeNull();
        expect(checkWinner({ 1: 5, 2: 0 }, 3)).toBeNull();
    });

    it("le joueur avec le plus de points gagne au tour 6", () => {
        expect(checkWinner({ 1: 7, 2: 3 }, 6)).toBe(1);
        expect(checkWinner({ 1: 2, 2: 6 }, 6)).toBe(2);
    });

    it("égalité de points au tour 6 donne un match nul", () => {
        expect(checkWinner({ 1: 4, 2: 4 }, 6)).toBe("draw");
    });

    it("la partie peut se terminer après le tour 6", () => {
