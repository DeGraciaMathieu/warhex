...
export function getCombatModifiers(attacker, target, state) {
    ...
    const attackerMods = [];
    const targetMods = [];
    if (hillKeys.has(ak)) attackerMods.push({ type: "bonus", label: "Bonus Colline (portée +1)", icon: "⛰" });
    if (townKeys.has(tk)) targetMods.push({ type: "bonus", label: "Bonus Couvert de ville (svg -1)", icon: "🏰" });
    if (forestKeys.has(tk)) targetMods.push({ type: "bonus", label: "Bonus Couvert de forêt (svg -1)", icon: "🌲" });
    if (riverKeys.has(tk)) targetMods.push({ type: "malus", label: "Malus Rivière (svg +1)", icon: "🏞" });
